...
///////////////////////////////OC
#pragma warning(once : 4311) //OC: to suppress warning due to pointer conversion
#pragma warning(once : 4312) //OC: to suppress warning due to pointer conversion
#define TRIMAXNUMSUBD 2000000//OC
extern int gErrorTRIANGLE;   //OC
///////////////////////////////OC
...

{
  ...
  if (badtri->key >= 1.0) {
    ...
    /*   fact and use the reciprocal of `badtri->key', which is > 1.0.   */
		...
		//OC: hack to prevent eventual division by 0 problem
		if(badtri->key == 0.)
		{
			length = 1.e+30;
		}
...

{
  ...
  if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
	...
	while ((m->badtriangles.items > 0) && (m->steinerleft != 0)) {
		...
		if(passCount > TRIMAXNUMSUBD) //OC
		{
			gErrorTRIANGLE = 1; break;
		} //OC
		passCount++;
	}
  }
